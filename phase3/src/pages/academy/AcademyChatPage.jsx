...
import { academyChatContactsRequest, academyConversationsRequest, academySendMessageRequest, academyThreadRequest } from "../../api/academy";
...
export default function AcademyChatPage() {
  ...
  const loadConversations = async ({ preserveSelection = true } = {}) => {
    ...
    setError("");
    try {
      const data = await academyConversationsRequest();
  ...
  useEffect(() => {
    async function loadContacts() {
      ...
      try {
        const data = await academyChatContactsRequest();
  ...
  useEffect(() => {
    async function loadThread() {
      ...
      try {
        const data = await academyThreadRequest(selectedPeerId, { limit: 30, offset: 0 });
  ...
  const handleSend = async (e) => {
    ...
    setSuccess("");
    try {
      await academySendMessageRequest({
      ...
      <div className="panel-grid">
        <article className="soft-card">
          ...
          {!loadingContacts && contacts.length === 0 ? (
            <p className="muted">No student contacts are available yet. Students will appear here after enrollment.</p>
          ) : null}

          {contacts.length > 0 ? (
            <div className="form-grid" style={{ marginTop: "16px" }}>
              <div>
                <label htmlFor="academyPeerSelect">Student</label>
                <select id="academyPeerSelect" value={selectedPeerId} onChange={(e) => setSelectedPeerId(e.target.value)}>
        ...
        <article className="soft-card">
          ...
          <form onSubmit={handleSend} className="form-grid" style={{ marginTop: "16px" }}>
            <div>
              <label htmlFor="academyMessageText">Message</label>
              <textarea
                id="academyMessageText"
                rows={4}
                value={messageText}
